...
const Layout = ({ children }:ContainerProps) => {
  ...
  const navigate = (path: string) => window.location.pathname = import.meta.env.BASE_URL + path;
  return (
    <div>
      <header className="grid grid-cols-2 fixed top-0 w-full z-20 align-middle h-[90px]">
        <a href={import.meta.env.BASE_URL}   className=" h-11 w-auto ml-[3%] self-center">
